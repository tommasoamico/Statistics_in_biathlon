...
years_women_path <- list.dirs(path='../biathlon_analysis/biathlon_data/men',
                              recursive=FALSE,full.names=TRUE)
...
years_women <- list.dirs(path = '../biathlon_analysis/biathlon_data/men', 
                         recursive =FALSE,full.names=FALSE)
...
locations_path[[years[i]]] <- (list.dirs(path = years_men_path[i], recursive = FALSE, full.names = TRUE))
}
```
...
for (i in seq_along(years)){
  for(j in seq_along(locations[[years[i]]])){
    files <- list.files(locations_path[[years[i]]],
    ...
    for (file in files){
      ...
      res_over <- res_over + length(df_res$Rank[destring(df_res$Rank) > 120]
             [! is.na(df_res$Rank[destring(df_res$Rank) > 120])])
      
      # Updating the DNS count
      res_dns <- res_dns + length(df_res$Rank[df_res$Rank=='DNS'])
      
      # Updating the DNF count
      res_dnf <- res_dnf + length(df_res$Rank[df_res$Rank=='DNF'])
      
      # Updating the DSQ count
      res_dsq <- res_dsq + length(df_res$Rank[df_res$Rank=='DSQ'])
      
      # Updating the lapped athletes' count
      res_lap <- res_lap + length(df_res$Rank[df_res$Rank=='LAP'])
     
      res_str <- res_str + length(df_res$Rank[(is.na(destring(df_res$Rank)))])
...
df_final_loop <- data.frame()

# Brginning the cycle that loops over every year and every venue
for (i in seq_along(years)){
    ...
    files <- list.files(locations_path[[years[i]]],
      ...
      for (file in files){
        ...
        if (
            ...
            # Adding the column with the id of the race
            df_loop$ID <- rep(race_id, nrow(df_loop))
          ...
          df_final_loop <- rbind(df_final_loop, df_loop)
...
if(res_dnf + res_dns + res_dsq + res_lap + res_over + nrow(df_final_loop) == nrow(df_results)){
