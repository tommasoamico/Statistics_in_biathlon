...
years_men_path <- list.dirs(path = '~/Desktop/biathlon_analysis/biathlon_data/men',
                            recursive=FALSE,full.names=TRUE)

# Path of the directory for each year for women
years_women_path <- list.dirs(path='~/Desktop/biathlon_analysis/biathlon_data/women',
                              recursive=FALSE,full.names=TRUE)

# List of years directory names for men
years_men <- list.dirs(path = '~/Desktop/biathlon_analysis/biathlon_data/men', 
                       recursive = FALSE, full.names=FALSE)

# List of years directory names for women
years_women <- list.dirs(path = '~/Desktop/biathlon_analysis/biathlon_data/women', 
                         recursive =FALSE,full.names=FALSE)
...
To retrieve the parameters of the beta from mode and standard deviation we needed to find numerically the root of the equation of the alpha parameter given mode and variance. 
...
plot <- ggplot() +
        ...
        theme(plot.title = element_text(size=20)) +
        theme(axis.title = element_text(size=15)) +
        labs(color = 'Probability \nColor map', x='Probability p', y='Prior PDF', title=
               'Priors computed - Men') 
...
```{r}
tmp <- data.frame(efficiency_list_w)
```
...
```{r}
colnames(tmp) <- purrr::map(.x = colnames(tmp), .f = replace_func)
```
...
```{r}
write.csv(tmp, file='~/Desktop/efficiency_w.csv', row.names = FALSE)
```

```{r}
read.table('~/Desktop/efficiency_w.csv', sep=',', header = TRUE)
```



```{r}
capture.output(efficiency_list,file =  '~/Desktop/efficiency_m.csv', fix.empty.names = FALSE)
```


```{r}
names(efficiency_list_w)
```


```{r}
# Definition of the Freedman Diaconis rule
